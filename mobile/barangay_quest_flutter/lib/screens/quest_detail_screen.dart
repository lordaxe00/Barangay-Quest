...
class _QuestDetailScreenState extends State<QuestDetailScreen> {
  ...
  Future<void> _apply() async {
    ...
    try {
      final data = d.data()!;
      // Prevent applying to your own quest
      final currentUser = FirebaseAuth.instance.currentUser;
      if (currentUser != null && data['questGiverId'] == currentUser.uid) {
        setState(() {
          _error = "You can't apply to your own job.";
        });
        return;
      }
      if ((data['status'] ?? 'open') != 'open') {
        setState(() {
          _error = 'This quest is not open for applications.';
        });
        return;
      }
      ...
      setState(() {
        _error = 'Failed to apply. Please try again.';
      });
    } finally {
      setState(() {
        _applyLoading = false;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text('Posted by ${data['questGiverName'] ?? 'User'}',
            ...
            Wrap(spacing: 12, runSpacing: 12, children: [
              ...
              _InfoChip(
                  label: 'Status',
                  value: data['status']?.toString().toUpperCase() ?? 'OPEN'),
              _InfoChip(
                  label: 'Location',
            ...
            const SizedBox(height: 8),
            // If user is the owner, show Applicants button. Otherwise show Apply button when open
            if (isOwner)
              FilledButton.tonal(
                onPressed: () =>
                    GoRouter.of(context).go('/quest/${d.id}/applicants'),
              ...
              FilledButton(
                ...
                child: _applyLoading
                    ...
                    : Text(_applied ? 'Application Submitted' : 'Apply Now'),
              ),
          ],
        ),
      ),
